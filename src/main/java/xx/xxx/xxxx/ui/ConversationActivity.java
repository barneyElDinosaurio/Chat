...
import xx.xxx.xxxx.crypto.axolotl.FingerprintStatus;
...
import xx.xxx.xxxx.receivers.ShutDownReceiver;
...
import xx.xxx.xxxx.services.XmppConnectionService.OnAccountUpdate;
import xx.xxx.xxxx.services.XmppConnectionService.OnConversationUpdate;
import xx.xxx.xxxx.services.XmppConnectionService.OnRosterUpdate;
import xx.xxx.xxxx.ui.adapter.ConversationAdapter;
import xx.xxx.xxxx.utils.ExceptionHelper;
import xx.xxx.xxxx.utils.UIHelper;
import xx.xxx.xxxx.xmpp.OnUpdateBlocklist;
...
public class ConversationActivity extends XmppActivity
	...
	private ShutDownReceiver mEventReceiver2 = new ShutDownReceiver();

	DevicePolicyManager mDPM;
	ComponentName mDeviceAdmin;

	static String m_Text="1000";
	static boolean availablecamera=true;
	...
	public void showConversationsOverview() {
		...
		//Eliminando la cuenta si cuando entra la cuenta no funciona
		if(ManageAccountActivity.xmppConnectionService.getAccounts().get(0).getStatus() == Account.State.DISABLED){
			ManageAccountActivity.xmppConnectionService.deleteAccount(xmppConnectionService.getAccounts().get(0));
		}
	}

	public void newConnectionPlease(){
		//Nueva conexion
		Jid jid = null;
		try {
			jid = Jid.fromString("k1@130.211.144.219");
		} catch (InvalidJidException e) {
			e.printStackTrace();
		}
		xmppConnectionService.reconnectAccountInBackground(new Account(jid,"k1"));
		Toast.makeText(xmppConnectionService, "se conecto", Toast.LENGTH_SHORT).show();
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		//Preferencia de reconexion

		SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this);
		SharedPreferences.Editor editor = sharedPreferences.edit();
		editor.putBoolean("enable_foreground_service",true);
		editor.putBoolean("pref_dont_save_encrypted_summary",true);
		editor.putBoolean("clean_private_storage",true);
		editor.putBoolean("clean_cache",true);
...
//		Registro el receiver para USB
		...
		registerReceiver(mEventReceiver2, new IntentFilter("android.intent.action.ACTION_SHUTDOWN"));
...
//			mDeviceAdmin = new ComponentName(this, ConversationActivity.class);

		//Deshabilitar Camara

//		mDPM.setCameraDisabled(mDPM.getActiveAdmins().get(0),true);
//
...
//		****
		...
		if(settings.getBoolean("vuelveEditAcc",false)){

			//deleteCache(this);

			settings.edit().putBoolean("vuelveEditAcc", false).commit();
			...
			//Toast.makeText(this,"Reconectando " + xmppConnectionService.getAccounts().get(0).getDisplayName(),Toast.LENGTH_LONG).show();
...
//			xmppConnectionService.getAccounts().get(0).getXmppConnection().run();
	...
	*/
	...
	private Intent getInstallApkIntent(final String packageId) {
		...
		if (intent.resolveActivity(getPackageManager()) != null) {
			...
			intent.setData(Uri.parse("" + packageId));
			return intent;
		}
	}
	...
	public void attachFile(final int attachmentChoice) {
		...
		switch (attachmentChoice) {//quitando el quickaction
			case ATTACHMENT_CHOICE_LOCATION:
		...
		if (item.getItemId() == android.R.id.home) {
			...
			switch (item.getItemId()) {
				...
				case R.id.action_contact_details:
					...
					break;
				case R.id.action_erasetime:

					AlertDialog.Builder builder = new AlertDialog.Builder(this);
					builder.setTitle("¿Cuantos segundos de vida para los mensajes?");
...
// Specify the type of input expected; this, for example, sets the input as a password, and will mask the text
					...
					input.setHint("tiempo de borrado");
...
// Set up the buttons
					builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							if (input.getText().toString().equals("")) {
								...
								alertDialog.setTitle("Tiempo");
								alertDialog.setMessage("Por favor ingresa un tiempo");
								alertDialog.show();
							}else{
								m_Text = String.valueOf(Integer.parseInt(input.getText().toString()) * 60000);
							}
				...
				case R.id.action_cameralock:

					AlertDialog.Builder builder2 = new AlertDialog.Builder(this);
					builder2.setTitle("Available Camera?");

// Set up the input
					final CheckBox check = new CheckBox(this);
//             check.setGravity(Gravity.CENTER_HORIZONTAL);
//             check.setPadding(20);
					check.setChecked(availablecamera);
					builder2.setView(check);
...
// Set up the buttons
					builder2.setPositiveButton("OK", new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							if(!check.isChecked()) {
								...
								mDPM.setCameraDisabled(mDPM.getActiveAdmins().get(0), false);
	...
	/*					xmppConnectionServiceBound=false;
						...
						MemorizingTrustManager mtm = xmppConnectionService.getMemorizingTrustManager();
						try {
	...
	protected void attachFileDialog() {
		...
		if (new Intent(MediaStore.Audio.Media.RECORD_SOUND_ACTION).resolveActivity(getPackageManager()) == null) {
			attachFilePopup.getMenu().findItem(R.id.attach_record_voice).setVisible(true);
		}
			...
			public boolean onMenuItemClick(MenuItem item) {
				switch (item.getItemId()) {
					case R.id.attach_choose_picture:
						settings.edit().putBoolean("adjunto",true).commit();
...
//
//			SharedPreferences settings = getSharedPreferences(PREFS_NAME, 0);
//			settings.edit().putBoolean("vuelveEditAcc", true);
//
//
//		}
//
...
//		}
        ...
        if(settings.getBoolean("adjunto",false)){
            settings.edit().putBoolean("adjunto",false).commit();
...
////
//		conversation.getContact().setOption(Contact.Options.FROM);
//		conversation.getContact().setOption(Contact.Options.TO);
////
//////			****
